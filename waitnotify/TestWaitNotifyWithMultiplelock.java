package com.test.multithread.waitnotify;

public class TestWaitNotifyWithMultiplelock {

    Object key = new Object();

    public static void main(String[] args) {
        // TODO Auto-generated method stub
        TestWaitNotifyWithMultiplelock test = new TestWaitNotifyWithMultiplelock();
        Thread t1 = new Thread(() -> test.execute1());
        Thread t2 = new Thread(() -> test.execute2());
        t1.start();
        t2.start();
    }

    public synchronized void execute1() {
        synchronized (key) {

            try {
                key.wait();
                System.out.println("haha");
            } catch (InterruptedException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
        }

    }

    public synchronized void execute2() {
        synchronized (key) {
            key.notify();
            //This will only release lock on key, not on this.
            //So there is dead lock.
        }

    }
}
